const galleryCloseBtn = document.querySelector('.show_close_btn')
const galleryOpenBtn = document.querySelector('.show_zoomout img.show_open_btn');
...
function galleryScroll(event){
	if(event.key == 'ArrowDown' && scrollBtnPoint == 'down' && isGalleryOpened == false){
		scrollBtnPoint = 'up'
		gsap.to('.gallery',{
			y: '-100vh',
			duration: '.6'
	...
	}else if(event.key == 'ArrowUp' && scrollBtnPoint == 'up' && isGalleryOpened == false){
		scrollBtnPoint = 'down'
		gsap.to('img.down',{
			rotate: '0deg',
			duration: '.6'
